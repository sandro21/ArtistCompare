...
const StickyArtistImages: React.FC<StickyArtistImagesProps> = ({ artistA, artistB }) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Get the viewport height and trigger much earlier (around 0.2 viewport heights)
  ...
  return (
    <div 
      className={`transition-all duration-500 ease-in-out z-40 hidden sm:block ${
        isSticky 
          ? 'fixed top-1/3 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-screen pointer-events-none'  
    ...
    >
      ...
      >
        <div className="flex flex-col items-center">
          ...
          <div 
            className="w-32 h-32 md:w-40 md:h-40 rounded-full overflow-hidden border-2 border-[#5EE9B5]"
            style={{
              boxShadow: '0 0 8px -2px #5EE9B5 inset, 0 0 10px 1px #5EE9B5'
            }}
          >
